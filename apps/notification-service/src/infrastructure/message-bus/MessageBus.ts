...
export class MessageBus {
  ...
  async connect(): Promise<void> {
    ...
    await this.channel.assertExchange(EXCHANGES.NOTIFICATION_EVENTS, 'topic', { durable: true });
  }
  ...
  async publish(event: DomainEvent): Promise<void> {
    ...
    const routingKey = `notification.${event.eventType.toLowerCase()}`;
    this.channel.publish(EXCHANGES.NOTIFICATION_EVENTS, routingKey, Buffer.from(JSON.stringify(envelope)));
  }
